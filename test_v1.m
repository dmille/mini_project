...
%         plot(center2(1), center2(2), '.r');
    ...
    viscircles([center_x, center_y], R, 'LineStyle','--', 'LineWidth', 0.5);
    
    center(1,i) = center_x;
    center(2,i) = center_y;
end
% plot(center(1,2:n-1), center(2,2:n-1), '.m');
...
for i=1:1
    ...
    plot(intersect1_x, intersect1_y,'or'); % Plot intersect1
%     plot([point(1,i) intersect1_x], [point(2,i) intersect1_y]); 
    ...
    plot(intersect2_x, intersect2_y,'og');
%     plot([point(1,i) intersect2_x], [point(2,i) intersect2_y]);
    
    % Useful tangent, eliminate the irrelavant tangent point
    vec_a = -[point(1,i) - intersect1_x, point(2,i) - intersect1_y];
    vec_b = -[point(1,i) - intersect2_x, point(2,i) - intersect2_y];
    
    temp1 = [point(1,i+1) + vec_a(1), point(2,i+1) + vec_a(2)];
    temp2 = [point(1,i+1) + vec_b(1), point(2,i+1) + vec_b(2)];
    plot(temp1(1), temp1(2),'om');
    plot(temp2(1), temp2(2),'ob');

%     temp_vec = [point(1,i+2) - point(1,i+1), point(2,i+2) - point(2,i+1)]
%     u = temp_vec ;
%     v = [center(1,i+1) - point(1,i+1), center(2,i+1) - point(1,i+1)]
%     KK = [u(1) u(2); v(1) v(2)]
...
% 
%     if(d1 < d2)
%         intersect = [intersect1_x, intersect1_y];
%     else
%         intersect = [intersect2_x, intersect2_y];
%     end
    
% % % % %     v = [point(1,i+2) - point(1,i+1), point(2,i+2) - point(2,i+1)];
% % % % %     
% % % % %     prj_temp1_v = dot(vec_a,v)/(norm(v)^2)*v;
% % % % %     prj_temp2_v = dot(vec_b,v)/(norm(v)^2)*v;
    
%     plot(point(1,i+1) + prj_temp1_v(1), point(2,i+1) + prj_temp1_v(2), 'ok');
%     plot(point(1,i+1) + prj_temp2_v(1), point(2,i+1) + prj_temp2_v(2), 'og');
    
% % % % %     u1 = [prj_temp1_v(1), prj_temp1_v(2)];
% % % % %     angle_1 = acosd((u1(1)*v(1) + u1(2)*v(2))/(norm(u1)*norm(v)));
% % % % %     
% % % % %     u2 = [prj_temp2_v(1), prj_temp2_v(2)];
% % % % %     angle_2 = acosd((u2(1)*v(1) + u2(2)*v(2))/(norm(u2)*norm(v)));
% % % % %     
% % % % %     if(angle_1 < angle_2)
% % % % %         intersect = [intersect1_x, intersect1_y];
% % % % %     else
...
%     plot(intersect(1),intersect(2), '.m', 'MarkerSize', 30)
%     plot([point(1,i) intersect(1)], [point(2,i) intersect(2)], '-k','LineWidth', 2);
% % % % %     in(1,1) = intersect(1);
% % % % %     in(2,1) = intersect(2);
% % % % %     intersect_store(1,i+1) = intersect(1);
% % % % %     intersect_store(2,i+1) = intersect(2);
    
end
% plot_arc(intersect, [point(1,2) point(2,2)], [center(1,2) center(2,2)], R)
...
for i=2:n-2
    ...
    plot(traverse_intersect(1,i), traverse_intersect(2,i), '*b')
%     plot([traverse_intersect(1,i) intersect_store(1,i+1)], [traverse_intersect(2,i) intersect_store(2,i+1)]);
end
...
for i=2:n-2
    u = [intersect_store(1,i+1) - point(1,i), intersect_store(2,i+1) - point(2,i)];
    v = [intersect_store(1,i+1) - traverse_intersect(1,i), intersect_store(2,i+1) - traverse_intersect(2,i)];
    angle_point_intersect = acosd((u(1)*v(1) + u(2)*v(2))/(norm(u)*norm(v)));
    

        line = line_param([intersect_store(1, i+1) intersect_store(2,i+1)],[traverse_intersect(1,i) traverse_intersect(2,i)]);
...
final_data = [final_data line_data{length(curve_data)+1}];
